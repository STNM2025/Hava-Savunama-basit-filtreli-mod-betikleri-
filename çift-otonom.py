...
class BalloonDetector:
    def __init__(self, arduino_port=None, frame_shape=(480, 640, 3)):
        ...
        self.center_point = (frame_shape[1] // 2, frame_shape[0] // 2)
    ...
    def draw_bounding_boxes(self, frame, mask_red, mask_blue):
        ...
        for contour in cv2.findContours(mask_red, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0]:
            if cv2.contourArea(contour) > 500:
                ...
                self.send_coordinates_to_arduino(center)
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)
                cv2.putText(frame, "Red Balloon", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)

                if self.is_center_inside_box(x, y, w, h):
                    if self.shot_timer is None:
                        self.shot_timer = time.time()
                    elif time.time() - self.shot_timer >= 1 and not self.shot_triggered:
                        self.send_shot_to_arduino()
    ...
    def is_center_inside_box(self, x, y, w, h):
        """Ekranın ortasındaki nokta bounding box içinde mi?"""
        return x <= self.center_point[0] <= x + w and y <= self.center_point[1] <= y + h

    def send_shot_to_arduino(self):
        """Arduino'ya atış verisi gönder ve terminale yazdır."""
        if self.arduino:
            try:
                message = "SHOT\n"
                self.arduino.write(message.encode('utf-8'))
                print("Ateşşşş!")
                print(f"Arduino'ya atış verisi gönderildi: {message}")
            except Exception as e:
                print(f"Arduino'ya atış verisi gönderme hatası: {e}")

    def send_coordinates_to_arduino(self, center):
        """Algılanan balonların koordinatlarını Arduino'ya gönder."""
        if self.arduino:
            try:
                x, y = center
                message = f"{x},{y}\n"
