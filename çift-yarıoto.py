...
class BalloonDetector:
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Manuel ateş komutu için 'f' tuşu kontrolü
                key = cv2.waitKey(1) & 0xFF
                if key == ord('f'):  # 'f' tuşuna basıldığında ateş komutu gönder
    ...
    def send_shot_to_arduino(self):
        """Arduino'ya manuel ateş komutu gönder."""
        if self.arduino:
            try:
                message = "SHOT\n"
                self.arduino.write(message.encode('utf-8'))
                print("Ateşşşş!")
                print(f"Arduino'ya ateş komutu gönderildi: {message}")
            except Exception as e:
                print(f"Arduino'ya ateş komutu gönderme hatası: {e}")
